...
function initObstacles() {
    ...
    for (let i = 0; i < 2; i++) {
        let x = i * 350;
        carsArray.push(new Obstacles(x, canvas.height - cell * 2 - 20, cell * 2, cell, 1, 'car'));
    }
    // вторая полоса препятствий
    for (let i = 0; i < 2; i++) {
        let x = i * 300;
        carsArray.push(new Obstacles(x, canvas.height - cell * 3 - 20, cell * 2, cell, -2, 'car'));
    }
    //третья полоса препятствий
    for (let i = 0; i < 2; i++) {
        let x = i * 400;
        carsArray.push(new Obstacles(x, canvas.height - cell * 4 - 20, cell * 2, cell, 2, 'car'));
    }
    //четвертая полоса препятствий
    for (let i = 0; i < 2; i++) {
        let x = i * 400;
        logsArray.push(new Obstacles(x, canvas.height - cell * 5 - 20, cell * 2, cell, -2, 'log'));
    }
    //пятая полоса препятствий
    for (let i = 0; i < 3; i++) {
        let x = i * 200;
        logsArray.push(new Obstacles(x, canvas.height - cell * 6 - 20, cell, cell, 1, 'turtle'));
    }
}
initObstacles();
